...
class EmploiTempsEtudiantContent extends StatefulWidget {
  const EmploiTempsEtudiantContent({super.key});

  @override
  State<EmploiTempsEtudiantContent> createState() => _EmploiTempsEtudiantContentState();
}

class _EmploiTempsEtudiantContentState extends State<EmploiTempsEtudiantContent> {
  final FirestoreService _firestoreService = FirestoreService();
  List<Map<String, dynamic>> _emploisAvecDetails = [];
  bool _isLoading = true;
  String? _errorMessage;
  String _vueActive = 'hebdomadaire';
  ...
  Future<void> _chargerEmploiTemps() async {
    try {
      ...
      final groupeId = etudiant.groupeId;

      // Charger l'emploi du temps du groupe
      final emploisStream = _firestoreService.getEmploiTempsAvecDetailsStream(groupeId);
  ...
  Widget _buildVueJournaliere() {
    final emploisDuJour = _trierEmploisParHeure(_getEmploisParJour(_selectedJour));
    ...
    return ListView.builder(
      itemCount: emploisDuJour.length,
      itemBuilder: (context, index) {
        final item = emploisDuJour[index];
        return _buildCreneauCard(item);
      },
    );
  }
                    ...
                    children: [
                      GestureDetector(
                        ...
                        child: Container(
                          ...
                          decoration: BoxDecoration(
                            color: _vueActive == 'hebdomadaire' ? Colors.orange[700] : Colors.transparent,
                            borderRadius: BorderRadius.circular(16),
                          ),
                          child: Icon(
                            Icons.calendar_today,
                            size: 16,
                            color: _vueActive == 'hebdomadaire' ? Colors.white : Colors.orange[700],
                          ),
                        ),
                      ),
                      GestureDetector(
                        ...
                        child: Container(
                          padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6),
                          decoration: BoxDecoration(
                            color: _vueActive == 'liste' ? Colors.orange[700] : Colors.transparent,
                            borderRadius: BorderRadius.circular(16),
                          ),
                          child: Icon(
                            Icons.list,
                            size: 16,
                            color: _vueActive == 'liste' ? Colors.white : Colors.orange[700],
                          ),
                        ),
                      ),
                      GestureDetector(
                        onTap: () => setState(() => _vueActive = 'journaliere'),
                        child: Container(
                          padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6),
                          decoration: BoxDecoration(
                            color: _vueActive == 'journaliere' ? Colors.orange[700] : Colors.transparent,
                            borderRadius: BorderRadius.circular(16),
                          ),
                          child: Icon(
                            Icons.today,
                            size: 16,
                            color: _vueActive == 'journaliere' ? Colors.white : Colors.orange[700],
                          ),
                        ),
                      ),
                    ],
                    ...
                    return Padding(
                      ...
                      child: ChoiceChip(
                        ...
                        labelStyle: TextStyle(
                          color: _selectedJour == jour ? Colors.white : Colors.black,
                        ),
                      ),
                    );
                  }).toList(),
